...
def create(out, font):
    ...
    d=canvas('concept_l3_cache','원본을 바꿔도 저장된 답은 잠시 남는다','10:00에 받은 답의 TTL은 300초 · 주소는 설명용 값',h=1210)
    for y,t,cache,original in [(280,'10:00','192.0.2.10 · TTL 300','192.0.2.10'),(620,'10:02','192.0.2.10 · TTL 180','192.0.2.20'),(960,'10:05 이후 새 질문','192.0.2.20 · 새 TTL','192.0.2.20')]:
        d.text(70,y-70,t,30,BLUE)
        d.pc(280,y,'PC',s=.63)
        d.server(835,y,'재귀 리졸버','저장된 A: '+cache,.75)
    ...
    d=canvas('concept_l3_changed','답이 달라지면 다음 접속 대상도 달라질 수 있다','주소는 설명용 값 · 변경이 정상인지 공격인지는 추가 근거로 판단한다',h=1060)
    for y,title,ip,color,server in [(340,'정상 기준','192.0.2.10',GREEN,'원래 웹 서버'),(740,'예상과 다른 응답','192.0.2.20',RED,'다른 웹 서버')]:
